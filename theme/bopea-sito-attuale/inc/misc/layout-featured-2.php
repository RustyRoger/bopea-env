<?php
if ( ! function_exists( 'bopea_feature_layout_2' ) ) {
	function bopea_feature_layout_2( $attrs ) {
		$module = shortcode_atts( array(
			'blockid'            => '',
			'section_style'      => 'jl_feature_2',
			'post_type'          => 'post',
			'term_slugs'         => '',
			'post_type_tax'      => '',
			'category'           => '',
		...
		<div class="jl_mb_wrap_f jl_clear_at">
			...
			<?php bopea_feature_2_listing( $module, $query_data ); ?>			
			</div>
		</div>			
			<?php
			wp_reset_postdata();			
...
if ( ! function_exists( 'bopea_feature_2_listing' ) ) :
	function bopea_feature_2_listing( $module = array(), $query_data = null ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			echo '<div class="jl_fr2_wrap"><div class="jl_fr2_inner">';
			$counter = 1;
			$get_main_ft = TRUE;
			while ( $query_data->have_posts() ) :
				$query_data->the_post();
				if($get_main_ft){
				jl_layout_m_r_2( $module );
				$get_main_ft = FALSE;
				}else{
				jl_layout_m_r_sm_2( $module );
				}
			endwhile;
			echo '</div></div>';			
		endif;
...
if ( ! function_exists( 'jl_layout_m_r_2' ) ) :
	function jl_layout_m_r_2( $module = array() ) {?>
		<div class="jl_p_fr2 jl_m_fr2 jl_ov_el jl_ov_mh">
...
if ( ! function_exists( 'jl_layout_m_r_sm_2' ) ) :
	function jl_layout_m_r_sm_2( $module = array() ) {?>
	<div class="jl_p_fr2 jl_ov_el jl_ov_sh jl_sm_mt">
			    <?php if ( has_post_thumbnail()) {?>
			        <div class="jl_img_holder">
				        <div class="jl_imgw">
				            <div class="jl_imgin">
