<?php
if ( ! function_exists( 'bopea_feature_main_ov_sm_li' ) ) {
	function bopea_feature_main_ov_sm_li( $attrs ) {
		$module = shortcode_atts( array(
			'blockid'            => '',
			'section_style'      => 'jl_main_ov_sm_li',
			'post_type'          => 'post',
			'term_slugs'         => '',
			'post_type_tax'      => '',
			'category'           => '',
		...
		<div class="jl_mb_wrap_f jl_wrap_eb jl_clear_at">
			<div class="<?php echo esc_attr( $atts_style ); ?>">				
				<?php bopea_main_ov_sm_li_listing( $module, $query_data ); ?>								
			</div>
...
if ( ! function_exists( 'bopea_main_ov_sm_li_listing' ) ) :
	function bopea_main_ov_sm_li_listing( $module = array(), $query_data = null ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			$counter = 0;
			$pnum = $query_data->post_count;
			echo '<div class="jl_fr16_wrap"> <div class="jl_fr16_inner">';
			while ( $query_data->have_posts() ) :
				$query_data->the_post();								
				if($counter == 0){
					jl_layout_main_ov_sm( $module );					
				}else{
					jl_layout_main_ov_sm_li( $module );
				}
				if(++$counter === $pnum) {}
			endwhile;
			echo '</div></div>';
...
if ( ! function_exists( 'jl_layout_main_ov_sm' ) ) :
	function jl_layout_main_ov_sm( $module = array() ) {?>
...
if ( ! function_exists( 'jl_layout_main_ov_sm_li' ) ) :
	function jl_layout_main_ov_sm_li( $module = array() ) {?>
