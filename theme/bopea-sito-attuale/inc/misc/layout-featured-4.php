<?php
if ( ! function_exists( 'bopea_feature_layout_4' ) ) {
	function bopea_feature_layout_4( $attrs ) {
		$module = shortcode_atts( array(
			'blockid'            => '',
			'section_style'      => 'jl_feature_4',
			'post_type'          => 'post',
			'term_slugs'         => '',
			'post_type_tax'      => '',
			'category'           => '',
		...
		<div class="jl_mb_wrap_f jl_clear_at">
			...
			<?php bopea_feature_4_listing( $module, $query_data ); ?>			
			</div>
		</div>		
			<?php
			wp_reset_postdata();			
...
if ( ! function_exists( 'bopea_feature_4_listing' ) ) :
	function bopea_feature_4_listing( $module = array(), $query_data = null ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			echo '<div class="jl_fr4_wrap"><div class="jl_fr4_inner">';
			$counter = 0;
			$pnum = $query_data->post_count;
			while ( $query_data->have_posts() ) :
				$query_data->the_post();								
				if($counter == 0){
					jl_layout_m_r_4( $module );		
				}else{
					jl_layout_m_r_sm_4( $module );					
				}
				$counter++;
			endwhile;
			echo '</div></div>';			
...
if ( ! function_exists( 'jl_layout_m_r_4' ) ) :
	function jl_layout_m_r_4( $module = array() ) {?>
		<div class="jl_p_fr4 jl_m_fr4 jl_ov_el jl_ov_mh">
...
if ( ! function_exists( 'jl_layout_m_r_sm_4' ) ) :
	function jl_layout_m_r_sm_4( $module = array() ) {?>
	<div class="jl_p_fr4 jl_ov_el jl_ov_sh jl_sm_mt">
			    <?php if ( has_post_thumbnail()) {?>
			        <div class="jl_img_holder">
				        <div class="jl_imgw">
				            <div class="jl_imgin">
