<?php
if ( ! function_exists( 'bopea_feature_layout_lig' ) ) {
	function bopea_feature_layout_lig( $attrs ) {
		$module = shortcode_atts( array(
			'blockid'            => '',
			'section_style'      => 'jl_feature_lig',
			'post_type'          => 'post',
			'term_slugs'         => '',
			'post_type_tax'      => '',
			'category'           => '',
		...
		<div class="jl_mb_wrap_f jl_wrap_eb jl_clear_at">
			<div class="<?php echo esc_attr( $atts_style ); ?>">				
				<?php bopea_feature_lig_listing( $module, $query_data ); ?>								
			</div>
...
if ( ! function_exists( 'bopea_feature_lig_listing' ) ) :
	function bopea_feature_lig_listing( $module = array(), $query_data = null ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			$counter = 0;
			$pnum = $query_data->post_count;
			echo '<div class="jl_frlig_wrap"> <div class="jl_frlig_inner">';
			while ( $query_data->have_posts() ) :
				$query_data->the_post();								
				if($counter == 0){
					jl_layout_m_r_lig( $module );		
					echo '<div class="jl_fli_con"><div class="jl_fli_wrap">';
				}else{
					jl_layout_m_r_sm_lig( $module );
				}
				if(++$counter === $pnum) {
   					 echo "</div></div>";
  				}
...
if ( ! function_exists( 'jl_layout_m_r_lig' ) ) :
	function jl_layout_m_r_lig( $module = array() ) {?>
		<div class="jl_en_lfr">
...
if ( ! function_exists( 'jl_layout_m_r_sm_lig' ) ) :
	function jl_layout_m_r_sm_lig( $module = array() ) {?>
