<?php
if ( ! function_exists( 'bopea_feature_layout_8' ) ) {
	function bopea_feature_layout_8( $attrs ) {
		$module = shortcode_atts( array(
			'blockid'            => '',
			'section_style'      => 'jl_feature_8',
			'post_type'          => 'post',
			'term_slugs'         => '',
			'post_type_tax'      => '',
			'category'           => '',
		...
		<div class="jl_mb_wrap_f jl_clear_at">
			<div class="<?php echo esc_attr( $atts_style ); ?>">
				<div class="jl_fr8_wrap">
					<div class="jl_fr8_inner">
						<?php bopea_feature_8_listing( $module, $query_data ); ?>			
					</div>	
				</div>
			</div>
		</div>			
...
if ( ! function_exists( 'bopea_feature_8_listing' ) ) :
	function bopea_feature_8_listing( $module = array(), $query_data = null ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			...
			while ( $query_data->have_posts() ) :
				$query_data->the_post();								
				if($counter == 0){
					jl_layout_m_r_8( $module );		
					echo '<div class="jl_fli_con"><div class="jl_fli_wrap">';
				}else{
					jl_layout_m_r_sm_8( $module );
				}
				if(++$counter === $pnum) {
   					 echo "</div></div>";
  				}
...
if ( ! function_exists( 'jl_layout_m_r_8' ) ) :
	function jl_layout_m_r_8( $module = array() ) {?>
		<div class="jlf8w <?php if ( 'yes' === $module['jl_en_frame'] ) {echo 'jl_en_fr jl_frg';} ?>">
			<div class="jl_p_fr8 jl_frsha">
...
if ( ! function_exists( 'jl_layout_m_r_sm_8' ) ) :
	function jl_layout_m_r_sm_8( $module = array() ) {?>
