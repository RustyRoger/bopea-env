<?php
if ( ! function_exists( 'bopea_feature_layout_17' ) ) {
	function bopea_feature_layout_17( $attrs ) {
		$module = shortcode_atts( array(
			'blockid'            => '',
			'section_style'      => 'jl_feature_17',
			'post_type'          => 'post',
			'term_slugs'         => '',
			'post_type_tax'      => '',
			'category'           => '',
		...
		<div class="jl_mb_wrap_f jl_wrap_eb jl_clear_at">
			<div class="<?php echo esc_attr( $atts_style ); ?>">				
				<?php bopea_feature_17_listing( $module, $query_data ); ?>								
			</div>
...
if ( ! function_exists( 'bopea_feature_17_listing' ) ) :
	function bopea_feature_17_listing( $module = array(), $query_data = null ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			$counter = 0;
			$pnum = $query_data->post_count;
			echo '<div class="jl_fr17_wrap"> <div class="jl_fr17_inner">';
			while ( $query_data->have_posts() ) :
				$query_data->the_post();								
				if($counter == 0 || $counter == 1){
					jl_layout_m_r_17( $module );					
				}else{
					jl_layout_m_r_sm_17( $module );
				}
				if(++$counter === $pnum) {}
			endwhile;
			echo '</div></div>';
...
if ( ! function_exists( 'jl_layout_m_r_17' ) ) :
	function jl_layout_m_r_17( $module = array() ) {?>
		<div class="jl_en_lfr">
...
if ( ! function_exists( 'jl_layout_m_r_sm_17' ) ) :
	function jl_layout_m_r_sm_17( $module = array() ) {?>
