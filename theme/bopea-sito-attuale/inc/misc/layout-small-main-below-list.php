<?php
if ( ! function_exists( 'bopea_feature_layout_16' ) ) {
	function bopea_feature_layout_16( $attrs ) {
		$module = shortcode_atts( array(
			'blockid'            => '',
			'section_style'      => 'jl_feature_16',
			'post_type'          => 'post',
			'term_slugs'         => '',
			'post_type_tax'      => '',
			'category'           => '',
		...
		<div class="jl_mb_wrap_f jl_wrap_eb jl_clear_at">
			<div class="<?php echo esc_attr( $atts_style ); ?>">				
				<?php bopea_feature_16_listing( $module, $query_data ); ?>								
			</div>
...
if ( ! function_exists( 'bopea_feature_16_listing' ) ) :
	function bopea_feature_16_listing( $module = array(), $query_data = null ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			...
			while ( $query_data->have_posts() ) :
				$query_data->the_post();								
				if($counter == 0){
					jl_layout_m_r_16( $module );					
				}else{
					jl_layout_m_r_sm_16( $module );
				}
				if(++$counter === $pnum) {}
			endwhile;
			echo '</div></div>';
...
if ( ! function_exists( 'jl_layout_m_r_16' ) ) :
	function jl_layout_m_r_16( $module = array() ) {?>
		<div class="jl_en_lfr">
...
if ( ! function_exists( 'jl_layout_m_r_sm_16' ) ) :
	function jl_layout_m_r_sm_16( $module = array() ) {?>
