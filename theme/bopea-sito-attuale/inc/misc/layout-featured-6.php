<?php
if ( ! function_exists( 'bopea_feature_layout_6' ) ) {
	function bopea_feature_layout_6( $attrs ) {
		$module = shortcode_atts( array(
			'blockid'            => '',
			'section_style'      => 'jl_feature_6',
			'post_type'          => 'post',
			'term_slugs'         => '',
			'post_type_tax'      => '',
			'category'           => '',
		...
		<div class="jl_mb_wrap_f jl_clear_at">
			...
			<?php bopea_feature_6_listing( $module, $query_data ); ?>			
			</div>
		</div>			
			<?php
			wp_reset_postdata();			
...
if ( ! function_exists( 'bopea_feature_6_listing' ) ) :
	function bopea_feature_6_listing( $module = array(), $query_data = null ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			echo '<div class="jl_fr6_wrap"><div class="jl_fr6_inner">';
			$counter = 1;
			$get_main_ft = TRUE;
			while ( $query_data->have_posts() ) :
				$query_data->the_post();
				if($get_main_ft){
				jl_layout_m_r_6( $module );
				$get_main_ft = FALSE;
				}else{
				jl_layout_m_r_sm_6( $module );
				}
			endwhile;
			echo '</div></div>';			
		endif;
...
if ( ! function_exists( 'jl_layout_m_r_6' ) ) :
	function jl_layout_m_r_6( $module = array() ) {?>
...
if ( ! function_exists( 'jl_layout_m_r_sm_6' ) ) :
	function jl_layout_m_r_sm_6( $module = array() ) {?>
	<div class="jl_p_fr6 jl_cgrid_layout jl_frsha jl_sm_mt">
			    ...
			    <div class="jl_fe_text">    
					...
					<?php if ( $module['jl_hsmt'] == 'yes' ) {?><p class="jl_fe_des"><?php echo wp_trim_words( get_the_excerpt(), 23, '...' );?> </p><?php }?>
			        <?php bopea_post_meta(get_the_ID());?>                
			    </div>
			</div>
	<?php }
