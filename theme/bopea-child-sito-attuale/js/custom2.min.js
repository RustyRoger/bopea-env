(function ($) {
    ...
    jl_nav_custom: function() {
            ...
            $('#mobile_menu_slide .menu-item-has-children > a').on("click", function () {
                ...
                $(this).find('.arrow_down i').toggleClass("jli-down-chevron").toggleClass("jli-up-chevron");
...
jlSpscript = (function($) {
    ...
    OptScript.trackPagenav = function (block) {
        ...
        var total_posts = (settings.page_max-settings.offset)* settings.posts_per_page;
        var max_offsets = settings.page_max-settings.offset;
        var pagecurrentcalc = Number((total_posts/settings.page_current+1).toFixed(3));
        var pagemaxcalc = Number((total_posts/(settings.page_max)).toFixed(3));
        /* if(settings.page_max <= 10){
            var pagemaxcalc = Number((total_posts/(settings.page_max)).toFixed(3));
        }
        else{
            var pagemaxcalc = Number((total_posts/(settings.page_max-1)).toFixed(3));
        }
        if(settings.page_max <= 6){
            var first_condition = settings.page_current > settings_max;
            console.log("HEY");
        }
        else{
            var first_condition = (pagecurrentcalc <= pagemaxcalc);
        } */
        var settings_max = ((settings.page_max * settings.posts_per_page) - settings.offset) / settings.posts_per_page;
        /* console.log("TOTAL POSTS / PAGECURRENT:"+ pagecurrentcalc);
        console.log("TOTAL POSTS / PAGEMAX:"+ pagemaxcalc);
        console.log("CONDITION: "+ (pagecurrentcalc <= pagemaxcalc));
        console.log(settings); */
        var ended = ((settings.page_max * settings.posts_per_page) - settings.offset);
        console.log(parseInt(settings.page_current)+1);
            console.log(parseInt(settings.page_current)+1);
            console.log(settings.page_max);
            console.log(settings_max);
        if(parseInt(settings.page_current)+1 == parseInt(settings.page_max) || settings.page_current >= settings.page_max || settings.page_max <= 1 || settings.page_current >= settings_max) {
